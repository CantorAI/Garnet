...
model = garnet.load_model(
    ...
    backend="tensorrt",
    entry_function="Qwen3DecodeBatch",
    weights=str(snapshots[-1]),
    input_shapes=[
        [batch_size, 1],
        [1, batch_size, 1],
        [layers, batch_size, page_size, kv_heads, head_dim],
        [layers, batch_size, page_size, kv_heads, head_dim],
        [batch_size, 1],
        [batch_size],
        [batch_size],
    ...
    cache_dir=str(SCRIPT_DIR / "qwen3_1_7b_decode_batch_b4_cache"),
    compile={
...
page_bits = np.zeros(
    (layers, batch_size, page_size, kv_heads, head_dim), dtype=np.uint16
)
...
position_ids = garnet.tensor_from_host(
    [0] * batch_size, dtype="int64", shape=[1, batch_size, 1]
)
...
page_table = garnet.tensor_from_host(
    list(range(batch_size)), dtype="int32", shape=[batch_size, 1]
)


def request(tokens, position):
    assert garnet.tensor_update_from_host(input_ids, tokens)
    assert garnet.tensor_update_from_host(
        position_ids, [position] * batch_size
    )
    assert garnet.tensor_update_from_host(
...
for position in range(14):
    ...
    result = model.forward(request(tokens, position))
...
steady = np.asarray(times[3:], dtype=np.float64)
aggregate_tokens_per_second = batch_size * 1000.0 / steady.mean()
assert np.isfinite(steady).all()
assert aggregate_tokens_per_second > 0.0
print(
    "Qwen3-1.7B B4 continuous decode passed: "
    f"load_ms={load_ms:.2f}, avg_step_ms={steady.mean():.3f}, "
    f"p95_step_ms={np.percentile(steady, 95):.3f}, "
